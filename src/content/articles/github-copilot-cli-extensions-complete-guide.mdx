...
devto_id: 3353461
devto_hash: 2df31ffdbe25c79b
hashnode_id: 69b631b37795ceb8f8fbaa92
hashnode_hash: 98062e91ca859a0f
---
...
This is what enables self-healing workflows. Your extension can watch for test failures, send the agent a message to fix them, wait for the response, and verify the fix — all programmatically. The most powerful pattern I've found is the **REPL loop**: listen for `session.idle`, run your validation (tests, lint, build), and if it fails, `session.send()` the failures back to the agent. It keeps looping until everything passes or hits a max iteration limit. I have a [full working example in the cookbook](/articles/copilot-cli-extensions-cookbook-examples).
