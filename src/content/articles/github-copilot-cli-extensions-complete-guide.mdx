...
Everything in this article uses the `@github/copilot-sdk/extension` import — the extension mode that attaches to a running CLI session. But the same [Copilot SDK](https://github.com/github/copilot-sdk) also has a **standalone mode** for embedding Copilot's agent runtime directly into your own applications. And that mode is available in **four languages**:
