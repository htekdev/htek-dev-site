...
description: "GitHub just shipped /fleet for Copilot CLI, enabling parallel multi-agent workflows. Plus 4 releases, built-in skills, and Copilot cloud agent goes beyond PRs."
pubDate: 2026-04-03
tags: ["GitHub Copilot", "Developer Experience", "AI", "Multi-Agent Systems"]
draft: false
---
...
You can also reference custom agents defined in `.github/agents/` to assign different models or tools to different tracks. Want a heavy model for complex logic and a light one for boilerplate docs? `/fleet Use @technical-writer.md as the agent for all docs tasks and the default agent for code changes.`
...
The release also includes 15+ UX fixes: autopilot no longer continues after Escape or Ctrl+C, keystrokes typed during loading are no longer lost, the CLI exits immediately after a session ends instead of waiting up to 10 seconds, and Home/End/Page Up/Page Down now work in the diff viewer.
...
Five days, four releases, a major blog post, and a significant expansion of Copilot cloud agent's capabilities. The pace is relentless, and the direction is clear: **GitHub is building a multi-agent development platform**.
...
The CLI shipped 27 features and 50+ fixes in five days. The tool is maturing fast, and the vision is becoming concrete: a multi-agent platform where developers orchestrate AI teams instead of writing every line themselves. That future is arriving sooner than I expected.
