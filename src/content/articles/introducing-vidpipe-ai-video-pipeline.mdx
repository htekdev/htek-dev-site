...
description: "I built an agentic video pipeline that turns recordings into shorts, captions, and posts. Now it's open source."
pubDate: 2026-02-14
tags: ["Open Source", "AI", "Automation", "GitHub Copilot"]
draft: false
---
...
| 3. **Silence Removal** | AI agent decides which pauses to cut (max 20%) |
...
- **SilenceRemovalAgent** — detects silence, decides which segments to cut based on context
- **ShortsAgent** — analyzes transcripts for viral moments, plans 6 multi-segment shorts
- **MediumVideoAgent** — identifies chapter-length content, assembles clips with transitions
...
## AI Silence Removal (Not the Dumb Kind)

Most silence removal tools are binary: detect silence, cut it out. This destroys pacing. A pause after a joke? Gone. A beat for emphasis? Deleted. Natural breathing room? Chopped.

vidpipe's **SilenceRemovalAgent** uses context-aware decision-making. It detects silence with FFmpeg's `silencedetect` filter, but then asks an AI agent: *Should this silence be removed?* The agent considers:

- Transcript context (is this mid-sentence or between topics?)
- Duration (0.5 seconds vs. 3 seconds)
- Position (intro, outro, or main content?)
- Accumulated removal (stays under 20% of total runtime)

The result is videos that feel tight but not frantic. You keep the natural rhythm. You lose the dead air.
...
The **ShortsAgent** identifies 6 viral moments. Each short runs 15–60 seconds, can span multiple non-contiguous segments, renders in 9:16 with captions, and comes with 5 platform-native posts. That's **30 ready-to-publish assets** from shorts alone.

## Medium Clips (Chapter-Length Content)

The **MediumVideoAgent** identifies complete ideas that need 1–3 minutes. Each clip spans a single topic, includes crossfade transitions if needed, renders in 16:9, and generates 5 platform-native posts. That's **15 more assets**.
