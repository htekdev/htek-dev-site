...
draft: false
devto_id: 3260923
devto_hash: ad65fcaf242e698e
hashnode_id: 69939a5d520e553a67c55802
hashnode_hash: c2e87db1ea81751d
---
...
1. **Check budgets** — has the agent exceeded its time, cost, or iteration limit?
2. **Compact context** — is the conversation approaching the token ceiling? If so, compress it automatically.
3. **Call the LLM** — send the accumulated conversation with tool definitions.
4. **Inspect the response** — check the output against blocked patterns. If the LLM says `DROP TABLE`, the iteration is killed before anything executes.
5. **Validate tool calls** — if the LLM requests a tool, check it against the allowlist. Blocked tools get a `[BLOCKED by harness]` message injected back into the conversation so the agent can adapt.
6. **Execute approved tools** — run the tool, then check the *output* against blocked patterns too. A tool that returns secrets gets caught here.
7. **Feed results back** — add the tool output to the conversation and loop back to step 1.
